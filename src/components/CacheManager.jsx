...
{/* 3 props:
- cacheSize – cate elemente (filme) sunt în cache.
- clearExpiredCache – o functie ce este apelată cand utilizatorul vrea să șteargă cache-ul mai vechi de 24h.
- clearCache – o functie ce va fi apelata cand utilizatorul vrea sa steargă intreg cache-ul. */}

const CacheManager = ({ cacheSize, clearExpiredCache, clearCache }) => {
  const [isExpanded, setIsExpanded] = useState(false); // pentru a verifica daca sectiunea "Setari avansate" e afisata (extinsa)
  
  return (
    <div className="bg-gray-900 rounded-md overflow-hidden transition-all duration-300 border border-gray-800">
     
      {/* Header-ul pe care il apasam*/}
      <button 
      ...
      <div 
        className={`overflow-hidden transition-all duration-300 ease-in-out ${
          isExpanded ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0' // face sectiunea vizibila daca isExpanded e true si o ascunde daca e false
        }`}
      >
